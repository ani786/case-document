...
import com.mydocumentsref.api.portal.internal.casedocumentservice.services.output.GetUnregisteredDocumentsResponse;
...
@IntegrationTest
@ActiveProfiles("dev")
public class GetManualTitlesControllerIntegrationTest {
    ...
    @Test
    public void getManualTitles() {
        HttpHeaders headers = new HttpHeaders();
        headers.add(Constants.X_ACTIONED_BY, "loggedInUser");
        headers.add(Constants.X_CORRELATION_ID, "getManualTitles");
        headers.setAccept(Collections.singletonList(MediaType.APPLICATION_JSON));

        ResponseEntity<GetUnregisteredDocumentsResponse> response = restTemplate.exchange(
                    createURLWithPort("/manual-titles/1"), HttpMethod.GET, new HttpEntity<>(headers),
