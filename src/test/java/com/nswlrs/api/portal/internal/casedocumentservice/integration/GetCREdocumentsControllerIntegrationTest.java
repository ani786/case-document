...
@IntegrationTest
@ActiveProfiles("dev")
public class GetCREdocumentsControllerIntegrationTest {
    ...
    @Test
    public void getCREdocuments() {
        HttpHeaders headers = new HttpHeaders();
        headers.add(Constants.X_ACTIONED_BY, "loggedInUser");
        headers.setAccept(Collections.singletonList(MediaType.APPLICATION_JSON));
        headers.add(Constants.X_CORRELATION_ID, "getCREdocuments");

        ResponseEntity<GetDealingsUsedResponse> response = restTemplate.exchange(
                    createURLWithPort("/cre-documents/1"), HttpMethod.GET, new HttpEntity<>(headers),
